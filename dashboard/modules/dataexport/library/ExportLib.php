...
class ExportLib
{
    ...
    protected static array $operatorAlias = [
        'ne'  => '<>',
        'eq'  => '=',
        'gt'  => '>',
        'egt' => '>=',
    ...
    public function __construct(int $id)
    {
        ...
        if ($this->row->where_field) {
            foreach ($this->row->where_field as $item) {
                $operator      = ExportLib::getOperatorByAlias($item['operator']);
    ...
    public static function getOperatorByAlias(string $operator)
    {
        return self::$operatorAlias[$operator] ?? $operator;
    }

    public static function getAliasByOperator(string $operator): string
    {
        $search = array_search($operator, self::$operatorAlias);
